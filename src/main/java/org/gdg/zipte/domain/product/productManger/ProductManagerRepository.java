...
public interface ProductManagerRepository extends JpaRepository<ProductManager, Long> {
    ...
    Optional<ProductManager> findByProductId(@Param("productId") Long productId);
    ...
    Optional<CategorySet> findCategoryByProductId(@Param("productId") Long productId);

    @Query("select pm from ProductManager pm where pm.product.id = :productId and pm.description = 'basic'")
    Optional<ProductManager> findBasicOne(@Param("productId") Long productId);
